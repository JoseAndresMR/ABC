...
class DQNAgent(object):
    ...
    def __init__(self, config, log_path, state_size, action_size, random_seed):
        ...
        self.memory = PrioritizedReplayBuffer(
            action_size, BUFFER_SIZE, BATCH_SIZE, random_seed, PRIORITIZED_ER_a)
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        td = self.td(state, action, reward, next_state, done) + \
            PRIORITIZED_ER_e
    ...
    def learn(self, experiences, gamma):
        ...
        local_argmax_actions = self.qnetwork_local(
            input).detach().argmax(dim=1).unsqueeze(1)
        next_qvalues = self.qnetwork_target(input).gather(
            1, local_argmax_actions).detach()
        ...
        Q_targets = Q_expected + \
            (Q_targets - Q_expected) * \
            (1/(len(self.memory)*probs.unsqueeze(1)))**PRIORITIZED_ER_b
    ...
    def td(self, state, action, reward, next_state, done):
        ...
        with torch.no_grad():
            ...
            next_inputs = {"state": (torch.from_numpy(
                next_state)).float().to(device)}
            # next_state = (torch.from_numpy(next_state)).float().to(device)
            q_target_next = self.qnetwork_target.forward(
                next_inputs).detach().max(1)
