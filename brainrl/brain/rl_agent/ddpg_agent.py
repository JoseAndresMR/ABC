...
class DDPGAgent(object):
    ...
    def __init__(self, config, log_path, state_size, action_size, random_seed):
        ...
        self.actor_local = NnModel(
            deepcopy(config["models"]["actor"]), sizes, random_seed).to(device)
        self.actor_target = NnModel(
            deepcopy(config["models"]["actor"]), sizes, random_seed).to(device)
        fake_inputs = {"state": torch.randn(1, state_size).to(
            device), "action": torch.randn(1, action_size).to(device)}
        writer = SummaryWriter(os.path.join(
            self.log_path, "neurons", "{}".format(config["ID"]), "actor_graph"))
        writer.add_graph(self.actor_local, fake_inputs)
        writer.close()
        self.actor_optimizer = optim.Adam(
            self.actor_local.parameters(), lr=self.metaparams["lr_actor"])

        # Critic Network (w/ Target Network)
        self.critic_local = NnModel(
            deepcopy(config["models"]["critic"]), sizes, random_seed).to(device)
        self.critic_target = NnModel(
            deepcopy(config["models"]["critic"]), sizes, random_seed).to(device)
        writer = SummaryWriter(os.path.join(
            self.log_path, "neurons", "{}".format(config["ID"]), "critic_graph"))
        writer.add_graph(self.critic_local, fake_inputs)
        writer.close()
        self.critic_optimizer = optim.Adam(
            self.critic_local.parameters(), lr=self.metaparams["lr_critic"])
        ...
        self.memory = ReplayBuffer(
            action_size, self.metaparams["buffer_size"], self.metaparams["batch_size"], random_seed)
    ...
    def step(self, states, actions, rewards, next_states, dones):
        ...
        self.learn_step = (self.learn_step +
                           1) % self.metaparams["learn_every"]
    ...
    def learn(self, experiences, gamma):
        ...
        Q_targets_next = self.critic_target(
            {"state": next_states, "action": actions_next})
        ...
        actor_loss = - \
            self.critic_local({"state": states, "action": actions_pred}).mean()
        # Minimize the loss
